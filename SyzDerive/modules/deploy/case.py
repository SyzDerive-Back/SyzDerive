...
stamp_bad_fuzzing = "BAD_FUZZING"
stamp_bad_deploy = "BAD_DEPLOY"
stamp_build_syzkaller = "BUILD_SYZKALLER"
stamp_build_kernel = "BUILD_KERNEL"
stamp_reproduce_ori_poc = "REPRO_ORI_POC"
stamp_case_basic_info_save = "FINISH_CASE_BASIC_INFO_SAVE"
...
class Case:
    def __init__(self, index, parallel_max, debug=False, force=False, port=53777, replay='incomplete', linux_index=-1, time=8, key_syscall=None, kernel_fuzzing=False, reproduce=False, alert=[], gdb_port=1235, qemu_monitor_port=9700, max_compiling_kernel=-1, store_read=True):
        ...
        self.arch = None
        self.compiler = None
        self.key_syscall = key_syscall
        self.kernel_fuzzing = kernel_fuzzing
        self.reproduce_ori_bug = reproduce
        self.alert = alert
    ...
    def init_logger(self, debug, hash_val=None):
        ...
        if hash_val != None:
            format = logging.Formatter('%(asctime)s Thread {}: {} %(message)s *@* %(module)s:%(funcName)s:%(lineno)d'.format(self.index, hash_val))
        else:
            format = logging.Formatter('%(asctime)s Thread {}: %(message)s *@* %(module)s:%(funcName)s:%(lineno)d'.format(self.index))
